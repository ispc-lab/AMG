    ...
    try:
        for it in range(1, config.train.max_iters + 1):
            ...
            if it % config.train.val_freq == 0 or it == config.train.max_iters:
                ...
                torch.save({
                    'config': config,
                    'ligand_atom_emb': model.ligand_atom_emb.state_dict(),
                    'ligand_encoder': model.ligand_encoder.state_dict(),
                    'embedding': model.embedding.state_dict(),
                    'W': model.W.state_dict(),
                    'W_o': model.W_o.state_dict(),
                    'alpha_mlp': model.alpha_mlp.state_dict(),
                    'comb_head': model.comb_head.state_dict(),
                    'focal_mlp_ligand': model.focal_mlp_ligand.state_dict(),
